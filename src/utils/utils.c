...
int check_args_manager(int argc, char *argv[], char** logfile, char** config_file, 
                        int *worker_limit, int *port, int *buffer_size)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (!strncmp(argv[i], "-p", 2)  && ((i == 7)     || 
            (i == 5 && found_worker_limit == FALSE))            && 
            (i + 1 < argc)){
            *port = atoi(argv[++i]);
        } else if (!strncmp(argv[i], "-b", 2)                       && 
            ((i == 9) || (i == 7 && found_worker_limit == FALSE))   && 
            (i + 1 < argc)){
